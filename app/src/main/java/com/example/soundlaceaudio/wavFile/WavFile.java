package com.example.soundlaceaudio.wavFile;
...
public class WavFile {
     ...
     * @throws IOException
     */
    public static WavFile openWavFile(File file) throws IOException, com.jlibrosa.audio.wavFile.WavFileException {
        ...
        if (bytesRead != 12) throw new com.jlibrosa.audio.wavFile.WavFileException("Not enough wav file bytes for header");
        ...
        if (riffChunkID != RIFF_CHUNK_ID)
            throw new com.jlibrosa.audio.wavFile.WavFileException("Invalid Wav Header data, incorrect riff chunk ID");
        if (riffTypeID != RIFF_TYPE_ID) throw new com.jlibrosa.audio.wavFile.WavFileException("Invalid Wav Header data, incorrect riff type ID");

        // Check that the file size matches the number of bytes listed in header
        if (file.length() != chunkSize + 8) {
            throw new com.jlibrosa.audio.wavFile.WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + file.length() + ")");
        }
        ...
        while (true) {
            ...
            if (bytesRead == -1) throw new com.jlibrosa.audio.wavFile.WavFileException("Reached end of file without finding format chunk");
            if (bytesRead != 8) throw new com.jlibrosa.audio.wavFile.WavFileException("Could not read chunk header");
            ...
            if (chunkID == FMT_CHUNK_ID) {
                ...
                if (compressionCode != 1)
                    throw new com.jlibrosa.audio.wavFile.WavFileException("Compression Code " + compressionCode + " not supported");
                ...
                if (wavFile.numChannels == 0)
                    throw new com.jlibrosa.audio.wavFile.WavFileException("Number of channels specified in header is equal to zero");
                if (wavFile.blockAlign == 0)
                    throw new com.jlibrosa.audio.wavFile.WavFileException("Block Align specified in header is equal to zero");
                if (wavFile.validBits < 2) throw new com.jlibrosa.audio.wavFile.WavFileException("Valid Bits specified in header is less than 2");
                if (wavFile.validBits > 64)
                    throw new com.jlibrosa.audio.wavFile.WavFileException("Valid Bits specified in header is greater than 64, this is greater than a long can hold");

                // Calculate the number of bytes required to hold 1 sample
                wavFile.bytesPerSample = (wavFile.validBits + 7) / 8;
                if (wavFile.bytesPerSample * wavFile.numChannels != wavFile.blockAlign)
                    throw new com.jlibrosa.audio.wavFile.WavFileException("Block Align does not agree with bytes required for validBits and number of channels");
                ...
                if (foundFormat == false) throw new com.jlibrosa.audio.wavFile.WavFileException("Data chunk found before Format chunk");

                // Check that the chunkSize (wav data length) is a multiple of the
                // block align (bytes per frame)
                if (chunkSize % wavFile.blockAlign != 0)
                    throw new com.jlibrosa.audio.wavFile.WavFileException("Data Chunk size is not multiple of Block Align");
        ...
        if (foundData == false) throw new com.jlibrosa.audio.wavFile.WavFileException("Did not find a data chunk");
     ...
     * @throws IOException
     */
    private double readSample() throws IOException, com.jlibrosa.audio.wavFile.WavFileException{
        long val = 0;

        for (int b = 0; b < bytesPerSample; b++) {
            if (bufferPointer == bytesRead) {
                int read = iStream.read(buffer, 0, BUFFER_SIZE);
                if (read == -1) throw new com.jlibrosa.audio.wavFile.WavFileException("Not enough data available");
                bytesRead = read;
                bufferPointer = 0;
            }
     ...
     * @throws IOException
     */
    public int readFrames(float[] sampleBuffer, int numFramesToRead) throws IOException, com.jlibrosa.audio.wavFile.WavFileException {
     ...
     * @throws IOException
     */
    private int readFramesInternal(float[] sampleBuffer, int offset, int numFramesToRead) throws IOException, com.jlibrosa.audio.wavFile.WavFileException {
     ...
     * @throws IOException
     */
    public long readFrames(float[][] sampleBuffer, int numFramesToRead, int frameOffset) throws IOException, com.jlibrosa.audio.wavFile.WavFileException {
     ...
     */
    private long readFramesInternal(float[][] sampleBuffer, int frameOffset, int numFramesToRead) throws IOException, com.jlibrosa.audio.wavFile.WavFileException{
        if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
